...
AS desconto,
    ...
    from comissoes_corretores_lancadas
        ...
        WHERE
        comissoes_corretores_lancadas.status_financeiro = 1 AND comissoes_corretores_lancadas.status_apto_pagar = 1 AND
        comissoes.user_id = {$id} AND month(data_baixa_finalizado) = {$mes}  AND comissoes.plano_id = 3 AND comissoes_corretores_lancadas.valor != 0
        ORDER BY comissoes.administradora_id
        ");
        } else {
            $dados = DB::select("
...
//        $comissa_lancada->save();
    ...
    public function criarPDFUser(Request $request)
    {
        ...
        SELECT
            ...
            WHERE
            comissoes_corretores_lancadas.status_financeiro = 1 AND comissoes_corretores_lancadas.status_apto_pagar = 1 AND
            comissoes.user_id = {$id} AND month(data_baixa_finalizado) = {$mes} AND valor != 0 ORDER BY comissoes.administradora_id
        ");
