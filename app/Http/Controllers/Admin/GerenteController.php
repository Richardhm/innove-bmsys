...
class GerenteController extends Controller
{
    ...
    public function totalizarMes(Request $request)
    {
        ...
        $total_empresarial = DB::select("
            ...
            WHERE comissoes.plano_id != 3 AND comissoes.plano_id != 1 AND comissoes_corretores_lancadas.status_apto_pagar = 1 and month(data_baixa_finalizado) = {$mes} and year(data_baixa_finalizado) = {$ano}
            ) AS plano1,
            (
            SELECT SUM(valor) AS total_plano3 FROM comissoes_corretores_lancadas
            INNER JOIN comissoes ON comissoes.id = comissoes_corretores_lancadas.comissoes_id
            WHERE comissoes.plano_id != 3 AND comissoes.plano_id != 1 AND comissoes_corretores_lancadas.estorno = 1 and month(data_baixa_estorno) = {$mes} and year(data_baixa_estorno) = {$ano}
            ) AS plano3
        ...
        $total_individual = DB::select("
            ...
            INNER JOIN comissoes ON comissoes.id = comissoes_corretores_lancadas.comissoes_id
            WHERE comissoes.plano_id = 1 AND comissoes_corretores_lancadas.status_apto_pagar = 1 and month(data_baixa_finalizado) = {$mes}  and month(data_baixa_finalizado) = {$ano}
            ) AS plano1,
            (
            SELECT
                SUM(valor)
            ...
            FROM comissoes_corretores_lancadas
            INNER JOIN comissoes ON comissoes.id = comissoes_corretores_lancadas.comissoes_id
            WHERE comissoes.plano_id = 1 AND comissoes_corretores_lancadas.estorno = 1 and month(data_baixa_estorno) = {$mes} and year(data_baixa_estorno) = {$ano}
            ) AS plano3;
        ...
        $total_coletivo = DB::select("
            ...
            FROM comissoes_corretores_lancadas
            INNER JOIN comissoes ON comissoes.id = comissoes_corretores_lancadas.comissoes_id
            WHERE comissoes.plano_id = 3 AND comissoes_corretores_lancadas.estorno = 1  and month(data_baixa_estorno) = {$mes} and year(data_baixa_estorno) = {$ano}
            ) AS plano3
    ...
    public function index()
    {
        ...
        if($folha_aberto->count() == 1) {
            ...
            $total_empresarial = DB::select("
                ...
                SELECT
                    SUM(valor)
                        AS total_plano1 FROM comissoes_corretores_lancadas
                INNER JOIN comissoes ON comissoes.id = comissoes_corretores_lancadas.comissoes_id
                WHERE comissoes.plano_id != 3 AND comissoes.plano_id != 1 AND comissoes_corretores_lancadas.status_apto_pagar = 1
                and month(data_baixa_finalizado) = {$mes} and year(data_baixa_finalizado) = {$ano}
                ) AS plano1,
                (
                SELECT
                    SUM(valor)
                ...
                INNER JOIN comissoes ON comissoes.id = comissoes_corretores_lancadas.comissoes_id
                WHERE comissoes.plano_id != 3 AND comissoes.plano_id != 1 AND comissoes_corretores_lancadas.estorno = 1
                and month(data_baixa_estorno) = {$mes} and year(data_baixa_finalizado) = {$ano}
                ) AS plano3
            ")[0]->total_empresarial_valor;
            ...
            $total_individual = DB::select("
                ...
                SELECT SUM(valor) AS total_plano1 FROM comissoes_corretores_lancadas
                INNER JOIN comissoes ON comissoes.id = comissoes_corretores_lancadas.comissoes_id
                WHERE comissoes.plano_id = 1 AND comissoes_corretores_lancadas.status_apto_pagar = 1
                ...
                SELECT SUM(valor) AS total_plano3 FROM comissoes_corretores_lancadas
                INNER JOIN comissoes ON comissoes.id = comissoes_corretores_lancadas.comissoes_id
                WHERE comissoes.plano_id = 1 AND comissoes_corretores_lancadas.estorno = 1
                and month(data_baixa_finalizado) = {$mes}
                and year(data_baixa_finalizado) = {$ano}
                ) AS plano3;
            ")[0]->total_individual_valor;
            ...
            FROM comissoes_corretores_lancadas
            INNER JOIN comissoes ON comissoes.id = comissoes_corretores_lancadas.comissoes_id
            WHERE comissoes.plano_id = 3 AND comissoes_corretores_lancadas.status_apto_pagar = 1
                    and month(data_baixa_finalizado) = {$mes}
            and year(data_baixa_finalizado) = {$ano}
            ) AS plano1,
            (
        ...
        $users = DB::select("
            ...
            WHERE (status_financeiro = 1 OR status_gerente = 1)
              AND finalizado != 1
              AND valor != 0
              AND (contrato_empresarial.financeiro_id != 12 || contratos.financeiro_id != 12)
              AND users.id NOT IN (
                SELECT user_id
                FROM valores_corretores_lancados
...
//        dd($users);
    ...
    public function infoCorretor(Request $request)
    {
        ...
        $valor_individual_a_receber = DB::select("
            ...
            comissoes.user_id = {$id} AND comissoes_corretores_lancadas.valor != 0 AND comissoes.plano_id = 1 and contratos.financeiro_id != 12
        ")[0]->total;
        ...
        $valor_coletivo_a_receber = DB::select("
            ...
            comissoes_corretores_lancadas.status_gerente = 0 AND
            comissoes_corretores_lancadas.status_apto_pagar != 1 AND
            comissoes.user_id = {$id} AND comissoes_corretores_lancadas.valor != 0 AND comissoes.plano_id = 3 AND contratos.financeiro_id != 12
        ")[0]->total;
        ...
        $total_coletivo = DB::select("
            ...
            SELECT
                ...
                AS total_plano1 FROM comissoes_corretores_lancadas
            INNER JOIN comissoes ON comissoes.id = comissoes_corretores_lancadas.comissoes_id
            WHERE comissoes.plano_id = 3 AND comissoes_corretores_lancadas.status_apto_pagar = 1 and month(data_baixa_finalizado) = {$mes} and year(data_baixa_finalizado) = {$ano} AND user_id = {$id}
            ) AS plano1,
            (
            SELECT
                COALESCE(SUM(CASE WHEN comissoes_corretores_lancadas.valor_pago THEN valor_pago ELSE valor END), 0)
            ...
            FROM comissoes_corretores_lancadas
            INNER JOIN comissoes ON comissoes.id = comissoes_corretores_lancadas.comissoes_id
            WHERE comissoes.plano_id = 3 AND comissoes_corretores_lancadas.estorno = 1 and month(data_baixa_estorno) = {$mes} and year(data_baixa_estorno) = {$ano} AND user_id = {$id}
            ) AS plano3
        ")[0]->total_coletivo_valor;
        ...
        $usuarios = DB::select("
            ...
            FROM comissoes_corretores_lancadas
                     ...
                     INNER JOIN users ON users.id = comissoes.user_id
            WHERE (status_financeiro = 1 or status_gerente = 1) and finalizado != 1 and valor != 0 AND contratos.financeiro_id != 12 and users.id NOT IN (SELECT user_id FROM valores_corretores_lancados WHERE MONTH(data) = {$mes} AND YEAR(data) = {$ano})
            GROUP BY users.id, users.name
            ORDER BY users.name;
         ");
    ...
    public function comissaoListagemConfirmadasMesEspecifico(Request $request)
    {
        ...
        $total_individual = DB::select("
            ...
            SELECT SUM(valor) AS total_plano1 FROM comissoes_corretores_lancadas
            INNER JOIN comissoes ON comissoes.id = comissoes_corretores_lancadas.comissoes_id
            WHERE comissoes.plano_id = 1 AND comissoes.user_id = {$id} AND comissoes_corretores_lancadas.status_apto_pagar = 1 and month(data_baixa_finalizado) = {$mes} AND year(data_baixa_finalizado) = {$ano}
            ) AS plano1,
            (
            SELECT SUM(valor) AS total_plano3 FROM comissoes_corretores_lancadas
            INNER JOIN comissoes ON comissoes.id = comissoes_corretores_lancadas.comissoes_id
            WHERE comissoes.plano_id = 1 AND comissoes.user_id = {$id} AND comissoes_corretores_lancadas.estorno = 1 and month(data_baixa_finalizado) = {$mes} AND year(data_baixa_finalizado) = {$ano}
            ) AS plano3;
        ")[0]->total_individual_valor;
        ...
        $total_coletivo = DB::select("
            ...
            FROM comissoes_corretores_lancadas
            INNER JOIN comissoes ON comissoes.id = comissoes_corretores_lancadas.comissoes_id
            WHERE comissoes.plano_id = 3 AND comissoes.user_id = {$id} AND comissoes_corretores_lancadas.status_apto_pagar = 1 and month(data_baixa_finalizado) = {$mes} AND year(data_baixa_finalizado) = {$ano}
            ) AS plano1,
            (
            SELECT
                COALESCE(SUM(CASE WHEN comissoes_corretores_lancadas.valor_pago THEN valor_pago ELSE valor END), 0)
            ...
            FROM comissoes_corretores_lancadas
            INNER JOIN comissoes ON comissoes.id = comissoes_corretores_lancadas.comissoes_id
            WHERE comissoes.plano_id = 3 AND comissoes.user_id = {$id} AND comissoes_corretores_lancadas.estorno = 1 and month(data_baixa_estorno) = {$mes} AND year(data_baixa_finalizado) = {$ano}
            ) AS plano3
        ")[0]->total_coletivo_valor;
...
AS desconto,
    ...
    from comissoes_corretores_lancadas
        ...
        WHERE
        comissoes_corretores_lancadas.status_financeiro = 1 AND comissoes_corretores_lancadas.status_apto_pagar = 1 AND
        comissoes.user_id = {$id} AND month(data_baixa_finalizado) = {$mes} AND year(data_baixa_finalizado) = {$ano}  AND comissoes.plano_id = 3 AND comissoes_corretores_lancadas.valor != 0
        ORDER BY comissoes.administradora_id
        ");
        } else {
            $dados = DB::select("
...
AS desconto,
    ...
    from comissoes_corretores_lancadas
        ...
        WHERE
        comissoes_corretores_lancadas.status_financeiro = 1 AND comissoes_corretores_lancadas.status_apto_pagar = 1 AND
        comissoes.user_id = {$id}  AND comissoes.plano_id = 3 AND comissoes_corretores_lancadas.valor != 0
        ORDER BY comissoes.administradora_id
        ");
        }
        return $dados;
...
//        $comissa_lancada->save();
    ...
    public function comissaoMesDiferente(Request $request)
    {
        ...
        $dados = DB::select("
               ...
               CASE
                ...
                comissoes_corretores_lancadas.status_gerente = 0 AND
                comissoes_corretores_lancadas.status_apto_pagar != 1 AND
                comissoes.user_id = {$id} AND comissoes_corretores_lancadas.valor != 0 AND contratos.plano_id = 1 and contratos.financeiro_id != 12
                ORDER BY comissoes.administradora_id
        ");
        return $dados;
    }
    ...
    public function coletivoAReceber(Request $request)
    {
        ...
        $dados = DB::select("
                ...
                comissoes_corretores_lancadas.status_gerente = 0 AND
                comissoes_corretores_lancadas.status_apto_pagar != 1 AND
                comissoes.user_id = {$id} AND comissoes_corretores_lancadas.valor != 0 AND contratos.plano_id = 3 and contratos.financeiro_id != 12
                ORDER BY comissoes.administradora_id
        ");
        return $dados;
    }
    ...
    public function aplicarDescontoCorretor(Request $request)
    {
        ...
        $comissoes_id = $ca->comissoes_id;
        $contrato_id = Comissoes::find($comissoes_id)->contrato_id;
    ...
    public function criarPDFUser(Request $request)
    {
        ...
        $total_empresarial = DB::select("SELECT	SUM((comissoes_corretores_lancadas.valor)) AS total FROM comissoes_corretores_lancadas INNER JOIN comissoes ON comissoes.id = comissoes_corretores_lancadas.comissoes_id WHERE comissoes_corretores_lancadas.status_financeiro = 1 AND comissoes_corretores_lancadas.status_apto_pagar = 1 AND comissoes.user_id = {$id} AND MONTH(data_baixa_finalizado) = {$mes} AND YEAR(data_baixa_finalizado) = {$ano} AND valor != 0")[0]->total;
        ...
        WHERE
            ...
            MONTH(data_baixa_finalizado) = {$mes} AND YEAR(data_baixa_finalizado) = {$ano} AND
            valor != 0 AND
    ...
    public function geralEstornoMes(Request $request)
    {
        ...
        $estorno = DB::select(
            ...
            (comissoes_corretores_lancadas.parcela) as parcela
            from comissoes_corretores_lancadas inner join comissoes on comissoes.id = comissoes_corretores_lancadas.comissoes_id
            where comissoes_corretores_lancadas.estorno = 1 and month(comissoes_corretores_lancadas.data_baixa_finalizado) = {$mes}"
